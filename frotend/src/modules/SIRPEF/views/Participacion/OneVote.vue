...
const { GetUser, result, hidden, popupStatus, popup, envioData, Loading, cortesia, cortesias, sendCortesia, loadCortesia} = useReportes()
...
<template>
  ...
  >
  <FormInput :FunGetUser="GetUser" :finger="true" :cort="true" @setCortesia="loadCortesia"/>
</Welcome>
...
<div id="contenedor">
  ...
  <section class="fixed bg-black bg-opacity-30 w-full h-full z-50 place-content-center top-0 left-0 cursor-pointer" v-if="cortesia" @click="(e:any) => {if(e.target.tagName == 'SECTION') cortesia = false}">
        <form class="w-[80%] md:w-[300px] mx-auto bg-white p-10 rounded-2xl" @submit.prevent="sendCortesia">
            <label class="my-4 block text-center text-xl">Cantidad de cortesía</label>
            <p class="my-2"><strong>Disponible: </strong> {{ cortesias.cortesias_faltantes }} 
              <br>
              <strong>Entregadas:</strong> {{ cortesias.cortesia_entregada }}</p>
            <input type="number" min="0" :max="cortesias.cortesias_faltantes" placeholder="0" class="text-center" name="cortesia"/>
            ...
            v-if="!Loading && cortesias.cortesias_faltantes > 0">Enviar</button>
        </form>
  </section>
  ...
  <main id="reportes">
    <section id="form">
      <section class="fixed bg-black bg-opacity-30 hidden w-full h-full z-50 place-content-center top-0 left-0 cursor-pointer" @click="(e) => hidden(e as PointerEvent)" ref="popup">
        

        <form @submit.prevent="envioData" class="bg-white dark:bg-slate-800 rounded-lg px-6 py-8 ring-1 ring-slate-900/5 shadow-xl w-96 fadeout" v-if="popupStatus >= 1">
            ...
            <input type="text" placeholder="Enfermo, en otro estado, etc..." name="motivo" minlength="15" pattern="^[a-zA-Z\s]*$" maxlength="255" required v-else> 
            <div class="grid place-items-center my-10" v-if="Loading">
                <svg aria-hidden="true" class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                    viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
