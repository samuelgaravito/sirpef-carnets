...
<template>
    <main>
        ...
        <Welcome
            ...
            <CardEvent 
            v-if="eventos.length > 0 && event_Active.id"
            :evento="eventos.find(e => e.id == event_Active.id)" 
            :id_event="id_event" 
            :ChangueEvent="ChangueEvent" 
            @SetIdToEdit="(id) => id_event_edit = id"
            @enterEvent="(evento) => enterEvent(evento)"
            @loadPersonal="(id) => id_event = id"
            :loadDes="true"
        ...
        <section v-if="id_event != 0" class="fixed bg-black bg-opacity-30 w-full h-full z-50 grid place-content-center top-0 left-0 cursor-pointer" @click="(e) => hidden(e as PointerEvent)">
