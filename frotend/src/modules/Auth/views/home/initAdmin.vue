...
<Welcome
  ...
  <main class="main relative" v-if="NameEvent.id">
    ...
    <FilteredDashboard 
        ...
        :tipoPersonas="[]"
        @setTipoPersona = "({tipo}) => tipoPersona = tipo"
...
<div class="w-[90%] md:h-auto xl:h-[18vh] mx-auto my-8" v-if="graphToSee.find(e => e == 1) && itemsCounted.length > 0">
  <CardsFedeVida :items="itemsCounted"/> 
</div>


    <section class="w-[90%] grid grid-cols-3 gap-3 mx-auto text-white fadeout text-center" id="Estadisticas">

      <article v-if="!NameEvent.whitCortesia && graphToSee.find(e => e == 1)" id="barras" class="md:h-[40vh] bg-white rounded-lg px-2 py-3 shadow-xl z-10 fadeout border col-start-1 col-end-2">
      ...
      <article v-else-if="NameEvent.whitCortesia && graphToSee.find(e => e == 1 && NameEvent.id)" class="md:h-[35vh] bg-white rounded-b-2xl text-white text-center md:rounded-2xl md:grid md:content-center rounded-xl p-2 shadow-lg border relative col-start-1 col-end-2">
      <Doughnut :data="DonutCortesias" :options="{maintainAspectRatio: false, plugins: { legend: {display: true}, datalabels: {display: true, color: 'white', font: {weight: 'bold', size: 20} ,formatter: (value) => value > 0 ? `${((value / DonutCortesias.datos.cortesia) * 100).toFixed(1)}%` : ''}}}" v-if="Object.keys(DonutCortesias).length > 0"/>
        <div v-if="Object.keys(DonutSex).length > 0" class="absolute pointer-events-none inset-0 flex items-center mt-10 justify-center text-2xl text-black">
          <p class="text-4xl font-bold md:text-1xl text-gray-600">{{ DonutCortesias.datos.cortesia.toLocaleString('es-ES', { useGrouping: true })}} 
          <small class="block w-full text-sm font-bold text-light-lavender mx-auto">Cortesias</small></p>
        </div>
      </article>

      <article v-if="graphToSee.find(e => e == 1)" class="dona_and_leyend md:h-[40vh] block md:flex items-center gap-2 rounded-xl bg-white text-[#010c41] p-2 shadow-lg border col-start-2 col-end-4 overflow-hidden">
        <div id="dona" class="relative mx-auto">
          <p class="absolute pointer-events-none inset-0 flex items-center justify-center text-2xl">{{DatosResumen.a ? ((DatosResumen.b[1]/ DatosResumen.a[1]) * 100).toFixed(2) : 0 }}%</p>
    ...
    <section>
      ...
      <div class="bg-white border p-10">
        <Participacion :enlaces="enlaces.filter((_, i) => i > 5)"/>
      </div>
      ...
      <div class="bg-white border p-10">
        <Participacion :enlaces="enlaces.filter((_, i) => i > 3 && i < 8)"/>
      </div>
    </section>
