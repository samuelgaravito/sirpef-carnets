...
import PhotoRegister from "../components/photoRegister.vue";
...
<template>
  ...
  <UploadPhoto v-if="personaCam != '0'" @clear="() => personaCam = '0'" :persona="personaCam"/>
        ...
        <table class="table-data">
          ...
          <tbody>
            <tr v-for="row in data.rows" :key="row.id" class="text-center">
              ...
              <td>
                {{ row.asistencias || "0"}} veces
              </td>

              <td>
                {{ row.lastEntry ? convertDateISO(row.lastEntry) : "Sin información"}}
              </td>
              ...
              <td v-if="store.authUser.isAdmin" class="w-[50px]"> 
                <ListOptions :row="row" :confirmacion="confirmacion" @setPersona="(new_persona) => persona = new_persona" :id_evento="eventActive.id" v-if="row.tipo_empleado_id == '7'"
                  @cam="({id}) => personaCam = id" :enableCam="true"
                />
            </td>
            </tr>
