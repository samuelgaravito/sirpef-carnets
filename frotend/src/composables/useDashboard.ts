...
const DonutCortesias = ref({
  ...
  const GetData = async () => {
    try {
      const url = `/api/registro/count/${FechaOn.value}/${FechaHas.value}`
  ...
  const GetTop5Unidades = async (SendNoti: boolean) => {
    try {
      ...
      Top_Unidades.value.menor = topMenor
      Top_Unidades.value.mayor = topMayor
      //RenderChartBar(info)
  ...
  const getItems = async () => {
      try {
          const response = await getItemsCounted(FechaOn.value, FechaHas.value, tipoPersona.value)
  ...
  const FilterPersonas = async (texto: string, total: number, unidad: string, uni_or_est: string) => {
    ...
    */
    ...
    if (texto == "Si Participó" || texto == "Si Asistió" || texto == "Rechazado" || texto == "No Asistió" 
      || uni_or_est == "Si Participó" || uni_or_est == "Rechazado"  || uni_or_est == "Si Asistió" || uni_or_est == "No Asistió") {
      voto_status = texto == "Si Participó" || uni_or_est == "Si Participó" || texto == "Si Asistió"  ? 1 : 0

    }
    else if(texto == "Participantes faltantes" || texto == "Participantes faltantes"|| texto == "Total de Participación" || uni_or_est == "Total de Participación") {
      voto_status = uni_or_est == "Participantes faltantes" || texto == "Participantes faltantes" ? 2 : 3
    } 
    else if(uni_or_est == 'unidades') {
      voto_status = 2
    }
    else {
      voto_status = 4
    ...
    const {data} = await Http.get(`/api/registro/get_per_x_unid/${unid_id}/${voto_status}/${FechaOn.value}/${FechaHas.value}`)
...
const ChangueDashboard = (value: number) => {
  ...
  //console.log(document.documentElement.scrollHeight - window.outerHeight)
