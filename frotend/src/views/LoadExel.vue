...
import UseEntes from "@/composables/votos/UseEntes"
...
const GetUnidAsc = async () => {
    ...
    selectedUnid.value = response.data.unidad_adscrita.find(element => element.active == true).nombre
  }
...
<template>
    ...
    <Welcome
        title="Carga masiva"
        subtitle="Cargue un archivo excel con la información del personal"
    >
    <form class="p-10 border rounded-2xl shadow col-span-2 bg-white" @submit.prevent="SendFile">
        ...
        <select name="ente_id" v-if="Entes.length > 0" class="rounded-3xl">
             <option value="">Seleccionar un ente</option>
             <option :value="ente.id" v-for="ente in Entes" :selected="ente.active == true">{{ ente.entes }}</option>
        </select>

        <!--<select name="unid_id" v-if="Unidades_Ascritas.length > 0" class="rounded-3xl">
             <option :value="unidad.id" v-for="unidad in Unidades_Ascritas" :selected="unidad.active == true" >{{ unidad.nombre }}</option>
        </select>-->
