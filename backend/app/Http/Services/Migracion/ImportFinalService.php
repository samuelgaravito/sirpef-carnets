...
class ImportFinalService
{
    ...
    static public function importFinal (Request $request) {
       ...
       foreach (array_slice($rows, 1) as $index => $row) {
           ...
           $data = [
               'cedula' => $cedula ? str_replace('.', '', $cedula) : '',
               ...
               'telefono' => $row[8],
               'telefono_2' => $row[9],
               'fecha_nacimiento' => $row[10] ? $row[10] : null,
               'correo_electronico' => null,
               'parroquia_id' => $row[11],
               'ministerio_id' => $row[12],
               'centro_id' => null,
           ];
           ...
           $personaExistente->estatus = $estatus ;
           $personaExistente->sexo = $row[5];
           $personaExistente->direccion = $row[6];
           $personaExistente->cargo = $row[7];
           $personaExistente->fecha_nacimiento = $row[10];
           $personaExistente->correo_electronico = null;
           $personaExistente->parroquia_id = $row[11];
           $personaExistente->ministerio_id = $row[12];
...
return response()->json($personas);
    ...
    static public function importFinalYARACUY(Request $request)
    {
        ...
        try {
            foreach (array_slice($rows, 1) as $index => $row) {
                ...
                $sexo = $row[6] ?? ''; // Columna 5: sexo
                $fecha_nacimiento = isset($row[5]) && !empty($row[5]) ? trim($row[5]) : null;
                ...
                $correo_electronico = isset($row[7]) ? trim($row[7]) : null; // Columna 7: correo electrónico
                ...
                $user_id = isset($row[11]) ? trim($row[11]) : null; // Columna 11: user_id
                $ministerio_id = !empty($row[12]) ? $row[12] : null; // Columna 12: ministerio_id
