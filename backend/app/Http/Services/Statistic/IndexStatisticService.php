...
class IndexStatisticService
{
    ...
    {
        $perPage = 10;  // Define la cantidad de resultados por página
        ...
        $personas->getCollection()->transform(function ($persona) {
            ...
            $registros = $persona->registrosEventoActivo()->where('voto', true);
            $lastEntry = $registros->orderBy('voto','asc')->first();

            $persona->participacion = $registro ? $registro->voto : null;
            $persona->asistencias = $registros->count();
            $persona->lastEntry =  $lastEntry ? $lastEntry->created_at : null;
            return $persona;
        });
