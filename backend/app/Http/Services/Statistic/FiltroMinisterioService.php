...
class FiltroMinisterioService
{
   ...
   // Función para desplegar lista de tipos de empleado
static public function tiposEmpleadoDelUsuario()
{
    // Obtener todos los tipos de empleado
    $tiposEmpleado = TipoEmpleado::all();

    // Mapear los tipos de empleado para la respuesta
    $tiposEmpleadoMapeados = $tiposEmpleado->map(function ($tipoEmpleado) {
        return [
            'id' => $tipoEmpleado->id,
            'nombre' => $tipoEmpleado->tipo,
        ];
    });

    return response()->json([
        'tipos_empleado' => $tiposEmpleadoMapeados,
    ]);
}
